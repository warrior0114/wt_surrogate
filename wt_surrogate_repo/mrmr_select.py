...
"""mRMR (MI-based) feature selection on NPZ dataset.

This file is a repo-root convenience entrypoint.
If you prefer, you can also run `python scripts/mrmr_select.py ...`.
"""
...
def main():
    ...
    ap.add_argument("--npz_dir", required=True, help="Directory containing per-case .npz files")
    ap.add_argument("--weights_dir", required=True, help="PIFENet weights directory (for on-the-fly extraction).")
    ap.add_argument("--device", default="cuda")
    ap.add_argument(
        "--desired_output",
        nargs="+",
        default=["RootFxb1", "RootMyb1", "TwrBsFxt", "TwrBsMyt", "TTDspFA", "GenPwr"],
    )
    ...
    with open(out_path, "w", encoding="utf-8") as f:
        json.dump(
            {"kept_feature_indices": kept_indices, "kept_feature_names": kept_names, "info": info},
            f,
            indent=2,
            ensure_ascii=False,
        )
